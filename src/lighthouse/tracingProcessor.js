...
// m65 and earlier
const SCHEDULABLE_TASK_TITLE_ALT3 =
...
export default class TraceProcessor {
  ...
  static getRiskToResponsiveness(
    ...
    return TraceProcessor._riskPercentiles(
      ret.durations,
      totalTime,
      percentiles,
      ret.clippedLength
  ...
  static getMainThreadTopLevelEvents(
    ...
    for (const event of tabTrace.mainThreadEvents) {
      ...
      const end =
        (event.ts + event.dur - tabTrace.navigationStartEvt.ts) / 1000;
  ...
  static findMainFrameIds(events) {
    ...
    if (
      ...
      log.info(
        `Detected renderer thread by 'TracingStartedInBrowser' event: pid ${pid}, tid ${tid}`
      );
    ...
    if (
      ...
      log.debug(
        `Detected renderer thread by 'TracingStartedInPage' event: pid ${pid}, tid ${tid}`
      );
