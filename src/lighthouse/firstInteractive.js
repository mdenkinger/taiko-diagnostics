...
export default class FirstInteractiveAudit {
  ...
  static getRequiredWindowSizeInMs(t) {
    ...
    const exponentiationComponent = Math.exp(
      EXPONENTIATION_COEFFICIENT * tInSeconds
    );
  ...
  static findQuietWindow(FMP, traceEnd, longTasks) {
    ...
    if (
      longTasks.length === 0 ||
      longTasks[0].start >
        FMP + FirstInteractiveAudit.getRequiredWindowSizeInMs(0)
    ) {
      return FMP;
    }
    ...
    for (let i = 0; i < longTasks.length; i++) {
      ...
      const windowSize = FirstInteractiveAudit.getRequiredWindowSizeInMs(
        windowStart - FMP
      );
      ...
      const taskClusters = FirstInteractiveAudit.getTaskClustersInWindow(
        longTasks,
        i + 1,
        windowEnd
      );
